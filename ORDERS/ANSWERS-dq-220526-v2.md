...
> **[Предложение 1]** Источники гидрологических данных: по замечанию, необходимо уточнить, что суточные уровни воды взяты с сайта [allrivers.info/about](https://allrivers.info/about), и для каждого гидропоста прописать конкретный источник (Киренск — Центр регистра и кадастра; остальные посты — Дальневосточное УГМС).
...
> **[Предложение 2]** Источник событийного архива ЧС: по замечанию, нужно уточнить источник — данные ЧС взяты с того же сайта, что и метеорологические данные ([meteo.ru/data/adverse-weather-conditions/](http://meteo.ru/data/adverse-weather-conditions/)).
...
> **[Предложение 3]** Требует пояснения: что конкретно имеется в виду под «пиковыми экстремумами», сглаженность которых является основанием для исключения месячного уровня агрегирования (см. конец абзаца ниже).
...
> **[Предложение 4]** Методический вопрос: по замечанию, данный раздел подлежит переосмыслению в логике LC-кривых (см. раздел «Методика построения LC-кривых» в репозитории [darachubarova/permafrost_analysis](https://github.com/darachubarova/permafrost_analysis)). Текущая формализация через lead-lag корреляцию рассматривается как предварительная.
...
> **[Предложение 5]** К рассмотрению: по замечанию, целесообразно провести аналогичный анализ для **зимнего сезона** — вместо суммы осадков использовать высоту снежного покрова; применять физическую зиму (декабрь текущего года относится к зиме следующего года).
...
> **[Предложение 6]** Требует пояснения: в тексте не раскрыто, как именно вычисляются аномально тёплые дни (рис. 2.5), дни с экстремальными осадками (рис. 2.6) и частота зимних/весенних оттепелей (рис. 2.7). Необходимо добавить определения для каждого из этих показателей.
...
> **[Предложение 7]** Требует уточнения: методика расчёта аномалий явно не описана в тексте. По замечанию, нужно пояснить, как вычисляются аномалии, с возможной ссылкой на ноутбук `permafrost_analysis.ipynb`.
...
> **[Предложение 8]** Требует пояснения: термин «гидропосты с экстремумами» в заголовке рис. 2.11 нуждается в определении. Также по замечанию — ЧС следует разделить по типам: отдельно заторы, отдельно паводки, отдельно половодье.
...
> **[Предложение 9]** Редакционная пометка: данная глава рассматривается как **предварительная**. По замечанию, финальную версию будут писать совместно с анализом LC-кривых. Текущее содержание не следует считать окончательным.
...
> **[Предложение 10]** Спорное содержание: по замечанию, типы «Криогенный наледный паводок / Зажоры» и «Абразия берегов / Термопросадки грунта» требуют верификации — таких событий на р. Лене либо не зафиксировано, либо они отсутствуют в исходной таблице данных.
...
> **[Предложение 11]** К рассмотрению: по замечанию, категории «чрезвычайная опасность» и «ветер» предлагается не включать в итоговый анализ ЧС.
...
> **[Предложение 12]** К рассмотрению: по замечанию, годовую динамику ЧС следует разделить по типам — отдельно заторы, отдельно паводки, отдельно половодье.

**Рисунок 3.6 — Годовая динамика количества ЧС (2008–2023)**  
Источник данных: `data/mchs_events_lena_bank.csv` (поле `date_start`).  
Период: 2008–2023 гг.  
Примечание (методика): линейный график; ось X — год, ось Y — число ЧС за год в береговой зоне р. Лены; единица наблюдения — одно верифицированное событие.  
![Рисунок 3.6 — Годовая динамика количества ЧС](../figures/fig_3_6_emergency_trend_2008_2023.png)
Интерпретация: временной ряд показывает межгодовую неравномерность нагрузки на подсистему реагирования. Годы локальных максимумов служат опорой для ретроспективной проверки сценариев управления. В сочетании с рис. 3.4–3.5 это формирует практическую основу сезонно-годового планирования.
