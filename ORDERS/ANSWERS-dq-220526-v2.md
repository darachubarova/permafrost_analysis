...
Для математической формализации временной структуры гидрометеорологических предикторов, гидрологических откликов и событийных сигналов в настоящей работе используется методика LC-кривых (Life Cycle Curves), заимствованная из раздела «Методика построения LC-кривых» PDF `ORDERS/статья_LC_финал_11_08.pdf` без изменения её базовой вычислительной схемы и адаптированная только на уровне предметной интерпретации гидрологических и событийных рядов.
...
Если $z_{\max}=z_{\min}$, ряд является константным и не используется для дифференцирующего LC-анализа, поскольку не содержит фазового смещения. В практической процедуре такие ряды исключаются из сравнительной выборки как неинформативные для выделения окна упреждения.
...
Использование четвёртой степени сохраняется в точном соответствии с PDF-источником: такое преобразование подавляет малые флуктуации и выделяет именно те участки, где происходит концентрация риска.
