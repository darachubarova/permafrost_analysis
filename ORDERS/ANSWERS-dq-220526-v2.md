...
Для математической формализации временной структуры гидрометеорологических предикторов, гидрологических откликов и событийных сигналов в настоящей работе используется методика LC-кривых (Life Cycle Curves), адаптированная по образцу раздела «Методика построения LC-кривых» из PDF `ORDERS/статья_LC_финал_11_08.pdf`.
...
Если $z_{\max}=z_{\min}$, ряд является константным и не используется для дифференцирующего LC-анализа, поскольку не содержит фазового смещения.
...
Такая процедура подавляет малые флуктуации и выделяет именно те участки, где происходит концентрация риска.
