...
| № | Вопрос |
|---|--------|
| [Предложение 1](#q230526-1) | Можно ли выполнить аналогичный анализ для зимнего сезона (физическая зима; снежный покров вместо осадков)? |
| [Предложение 2](#q230526-2) | Описать числовые пороги классификации аномалий (аномально тёплые дни, экстремальные осадки, оттепели) или убрать размытую формулировку из текста. |
| [Предложение 3](#q230526-3) | ФТ просит разделить ЧС на Рис. 2.11 по типам (заторы / паводки / половодье). Изменятся ли графики? |
| [Предложение 4](#q230526-4) | Таблица 3.2 — убрать несуществующие типы: Зажоры, Криогенный наледный паводок, Абразия берегов, Термопросадки. |
...
| [3](#proposal-3) | Что такое «пиковые экстремумы»; обоснование исключения месячного шага | §2.2 |
...
## Протокол обсуждения (хронология замечаний, 22.05.26)

> Нумерация ниже отражает порядок поступления замечаний на обсуждении и **не совпадает** с нумерацией итоговых предложений в навигационной таблице выше.

[**Замечание 1. 12:05** — Описать источники данных → §2.1](#sec-21)
...
[**Замечание 2а. 12:05** — Убрать Верхоянск/Оймякон → §2.2](#sec-22)

Предлагается убрать Оймякон и Верхоянск из данных, так как они находятся на реках Яна и Индигирка

[**Замечание 2б. 12:10** — Погодичные таблицы по 4 станциям → §2.4](#sec-24)
...
[**Замечание 3. 12:30** — Источники, период 2008–2023, статистики → §2.1, §2.2, §2.4](#sec-21)
...
[**Замечание 4. 12:35** — Расчётный период 2008–2023 → §2.2](#sec-22)
...
[**Замечание 5. 12:40** — Уровни агрегирования → §2.2](#sec-22)

"Уровни агрегирования: день, месяц, год."

У нас нет месячных данных

[**Замечание 6. 12:50** — Статистики за 2008–2023 и bar-plot → §2.4](#sec-24)
...
[**Замечание 7. 13:00** — Порядок гидропостов по течению → §3.1](#sec-31)
...
[**Замечание 8. 13:00** — Площадки CALM (R42/R43) → §2.2](#sec-22)
...
[**Замечание 9. 13:05** — Частота встречаемости ЧС → §3.3](#sec-33)
...
- [**§ 3.3**](#sec-33) — Анализ 17 верифицированных ЧС за 2008–2023 гг.: 41% — заторы (май), 29% — половодье (май), 24% — пожароопасность (июнь–август), 6% — паводок. Вывод: концентрация ресурсов МЧС на май и начало лета.
...
> **[Предложение 10]** Таблица 3.2 приведена в соответствие с первичным архивом `data/mchs_events_lena_bank.csv`. Категории «Зажор» и «Абразия берегов», отсутствующие в верифицированном архиве МЧС за 2008–2023 гг., удалены. Итоговый набор: 17 событий в 4 категориях (Затор, Половодье, Пожароопасность, Паводок).
...
> **[Предложение 12]** Постановка уточнена: итоговая визуализация годовой динамики ЧС представлена в разрезе 4 типов — отдельные ряды для Затора, Половодья, Паводка и Пожароопасности вместо одного суммарного ряда. Фильтр исключения «Ветра» применён во всех трёх функциях (Рис. 3.4, 3.5, 3.6).
...
- [x] Таблица 3.2 приведена в соответствие с архивом `data/mchs_events_lena_bank.csv`: 17 событий, 4 категории (Затор, Половодье, Пожароопасность, Паводок); категории «Зажор» и «Абразия берегов» удалены как отсутствующие в архиве МЧС.
- [x] Зафиксировано исключение категории «Ветер» из итогового набора анализируемых ЧС; фильтр реализован в `scripts/build_figures_dq220526.py`.
- [x] Для годовой динамики ЧС закреплена итоговая постановка с раздельными рядами по типам (Затор, Половодье, Пожароопасность, Паводок); функция `fig_emergency_trend` переписана.

---

## Ответы на вопросы 23.05.2026

<a id="q230526-1"></a>

### Предложение 1. Зимний сезонный анализ (физическая зима; снежный покров)

**Вопрос:** Описательные характеристики климатических параметров весеннего периода по годам (2008–2023 гг.) — можно ли сделать такой анализ для зимнего сезона? Вместо уровня осадков брать высоту снежного покрова. При этом рассматривать не календарную зиму, а физическую: например, декабрь 2007 года относить к зиме 2008 года.

**Ответ:**

**Да, анализ технически выполним и частично уже реализован в ноутбуке.**

#### 1. Данные

Данные высоты снежного покрова хранятся в `data/snow/` в формате ВНИИГМИ-МЦД (`idata=7`): суточные значения высоты снега (см) и степени покрытия для каждой станции. Файл данных — `wr373144a5` (архив). Парсер уже написан: `parse_snow_file()` (ячейка 13 ноутбука).

#### 2. Определение физической зимы — уже реализовано

В функции `compute_degree_days()` (ячейка 33) переменная `snow_season` определяется именно по принципу физической зимы:

```python
# Ноябрь и декабрь года N → зимний сезон N+1
station_snow["snow_season"] = station_snow.apply(
    lambda row: row["year"] + 1 if row["month"] in (11, 12) else row["year"],
    axis=1,
)
```

Таким образом, **декабрь 2007 года уже относится к зиме 2008 года** в текущем коде. Месяцы снегонакопления: ноябрь–апрель (`[11, 12, 1, 2, 3, 4]`).

#### 3. Какие характеристики можно получить по зимним сезонам (2008–2023)

| Показатель | Описание | Связь с ММГ |
|-----------|----------|-------------|
| `max_snow_cm` | Максимальная высота снежного покрова за сезон | Снег — теплоизолятор: высокий покров снижает FDD и уменьшает глубину промерзания |
| `snow_cover_days` | Число дней со снежным покровом > 0 см | Продолжительность изоляции грунта от морозов |
| `first_snow_date` | Дата установления устойчивого покрова (≥ 3 дней подряд с h > 0) | Ранний снег осенью — защита ММГ от промерзания |
| `last_snow_date` | Дата схода снежного покрова весной | Поздний сход → отсрочка протайки СТС |
| `mean_snow_cm` | Средняя высота за сезон | Интегральная тепловая защита |
| `FDD` | Сумма градусо-дней мороза за зимний сезон | Уже рассчитывается; определяет глубину сезонного промерзания |

#### 4. Что нужно добавить

Отдельная таблица описательной статистики по зимним сезонам (аналог весенней) требует небольшого дополнения к функции `compute_degree_days()` — добавить расчёт `first_snow_date`, `last_snow_date`, `snow_cover_days`. Сейчас уже рассчитываются `max_snow_cm` и `mean_snow_cm`.

Новый рисунок (условно Рис. 2.X) мог бы показывать межгодовую изменчивость максимальной высоты снежного покрова по станциям (2008–2023) — прямой аналог Рис. 2.5 (аномально тёплые дни), но для зимнего сезона.

#### 5. Методическое замечание

Физическая зима в тексте ВКР должна быть явно определена: «зима года N = декабрь (N-1) + январь–март N». Это стандартное метеорологическое определение. Текущий код использует ноябрь–апрель как сезон снегонакопления — это шире метеорологической зимы, но оправдано для оценки теплозащитного эффекта снежного покрова на ММГ.

---

<a id="q230526-2"></a>

### Предложение 2. Числовые пороги классификации аномалий

**Вопрос:** В тексте сказано, что «числовые значения порогов не фиксируются… поскольку они параметризуются в ноутбуке по типу показателя». Это вызовет вопросы ФТ. Нужно либо описать пороги явно, либо убрать эту фразу.

**Ответ: Описываем явно.** Все пороги зафиксированы в ноутбуке `NOPik/permafrost_analysis.ipynb` и не меняются между запусками. Ниже — полное описание методики по каждому типу аномалии.

---

#### А. Аномально тёплые дни

**Метод:** суточная климатология с плавающим окном ±7 дней (ячейка 39, функция `_daily_climatology_vectorized`).

Для каждого дня года (1–366) строится историческая выборка: берутся все наблюдения среднесуточной температуры `tmean` в окне ±7 дней от данного дня года **по всем годам базового периода**. Итого окно шириной 15 дней × 16 лет ≈ 240 значений на каждый день года.

По этой выборке вычисляются:
- `tmean_norm` — историческое среднее (норма дня года)
- `tmean_std` — стандартное отклонение

**Порог:** день считается аномально тёплым, если:

$$z = \frac{T_{\text{сут}} - T_{\text{норм}}}{\sigma} \geq 2{,}0$$

При z ≥ 2,0 — **умеренная аномалия** (`severity = умеренная`, `risk_points = 1`).  
При z ≥ 3,0 — **экстремальная аномалия** (`severity = экстремальная`, `risk_points = 2`).

Дополнительно проверяется скользящий z-счёт по предыдущим 7 годам (`rolling_z`): порог тот же (≥ 2,0 / ≥ 3,0). Итоговый сигнал выставляется, если **хотя бы один** из двух критериев выполнен.

---

#### Б. Дни с экстремальными осадками

**Метод:** wet-day percentile (стандарт ETCCDI) — ячейка 39, та же функция.

Для каждого дня года аналогично строится историческая выборка осадков за ±7 дней. Из неё отбираются только **влажные дни** (`precip_mm > 0`):

```python
p_wet = p_series[p_series > 0]
precip_p95 = p_wet.quantile(0.95)
```

**Порог:** день с экстремальными осадками — если `precip_mm ≥ precip_p95` (wet-day P95 дня года).

Использование wet-day P95 вместо all-day P95 принципиально для субарктического климата: в зимние месяцы большинство дней имеют 0 мм осадков, поэтому all-day P95 ≈ 1–2 мм — любой снегопад ложно попадал бы в «экстремальные». Wet-day P95 даёт реалистичный порог ~5–15 мм в зависимости от сезона и станции.

---

#### В. Частота зимних оттепелей

**Метод 1 — счётчик дней** (ячейка 33, функция `compute_degree_days`):

`winter_thaw_days` — число дней в зимнем сезоне (октябрь – март, физическая зима), когда среднесуточная температура **превысила 0 °C**:

```python
thaw = winter.assign(v=(winter["tmean"] > 0))
             .groupby("winter_season")["v"].sum()
```

Порог фиксированный: **0 °C** (физический переход через точку таяния льда).

**Метод 2 — дата устойчивой весенней оттепели** (ячейка 42, функция `detect_thaw_onset`):

Первый день, начиная с **1 марта**, с которого **7-дневная скользящая средняя** `tmean` превышает 0 °C и удерживается не менее **5 дней подряд**. Параметры (ячейка 24):

| Константа | Значение | Смысл |
|-----------|----------|-------|
| `THAW_ROLLING_DAYS` | 7 | Ширина скользящего окна, дней |
| `THAW_STREAK_DAYS` | 5 | Минимальная серия тёплых дней |
| `THAW_MONTH_START` | 3 (март) | Начало весеннего окна |
| `THAW_MONTH_END` | 5 (май) | Конец весеннего окна |

---

#### Г. Итоговая таблица всех порогов

| Тип аномалии | Показатель | Порог умеренной | Порог экстремальной | Источник |
|-------------|-----------|----------------|---------------------|----------|
| Аномально тёплый день | z-счёт tmean | z ≥ 2,0 | z ≥ 3,0 | Ячейка 37, 39 |
| Экстремальные осадки | precip vs wet-day P95 | precip ≥ P95 | precip ≥ P99 | Ячейка 39 |
| Зимняя оттепель | tmean > 0 °C | любой день > 0 °C | — | Ячейка 33 |
| Весенняя оттепель (дата) | 7-дн. скользящая > 0 °C | ≥ 5 дней подряд | — | Ячейка 42 |

**Вывод:** вместо размытой формулировки «пороги параметризуются и подлежат обновлению» — использовать приведённую выше таблицу в §2.5 текста ВКР. Пороги стабильны и обоснованы методически.

---

<a id="q230526-3"></a>

### Предложение 3. Разбивка ЧС по типам на Рис. 2.11

**Вопрос:** ФТ просит разделить ЧС на Рис. 2.11 по типам (заторы / паводки / половодье). Изменятся ли графики?

**Ответ:**

#### Что сейчас на Рис. 2.11

Рисунок — двухосевой комбинированный график:
- **Левая ось (зелёная линия):** доля гидропостов с экстремумами (из `результаты/water_chs_link_by_year.csv`, поле `hydro_extreme_share`)
- **Правая ось (оранжевая линия):** суммарное число ЧС по году (`chs_count`) — агрегат из **полного** архива МЧС по Якутии (`результаты/mchs_events.csv`, все типы, вся территория)

Источник: функция `fig_hydro_share_vs_chs()` в `scripts/build_figures_dq220526.py`.

#### Почему разбивка по типам здесь проблематична

**Ключевое противоречие:** `chs_count` на Рис. 2.11 считается из **полного** архива МЧС (`mchs_events.csv` — ~145 событий, вся Якутия, 1991–2024), а не из берегового контура `mchs_events_lena_bank.csv`. В полном архиве более 10 типов ЧС (пожары, ветер, засухи и т.д.) — разбивка по типам превратит рисунок в нечитаемое многострочное полотно.

**Дублирование с Рис. 3.6:** именно эту задачу — показать годовую динамику ЧС по типам — уже решает **Рис. 3.6** (`fig_emergency_trend`), построенный по `mchs_events_lena_bank.csv` с 4 типами (Затор, Половодье, Паводок, Пожароопасность).

#### Рекомендация

Разбивку по типам на Рис. 2.11 **не делать**. Вместо этого — чётко разграничить назначение двух рисунков в тексте:

| Рисунок | Цель | Источник данных | Разбивка по типам |
|---------|------|-----------------|-------------------|
| **2.11** | Корреляция: гидроэкстремумы ↔ общий уровень ЧС по годам | `mchs_events.csv` (полный, вся Якутия) | Нет — намеренно: показывает суммарный сигнал |
| **3.6** | Структура ЧС на берегу Лены по типам | `mchs_events_lena_bank.csv` (17 событий) | Да — 4 типа |

**Формулировка для ФТ:** «Разбивка по типам представлена на Рис. 3.6, построенном по верифицированному набору событий на береговом контуре р. Лены. Рис. 2.11 намеренно использует агрегированный показатель по полному архиву МЧС — это позволяет оценить синхронность гидрологических экстремумов и общего уровня чрезвычайных ситуаций в регионе, независимо от их типологии.»

#### Если ФТ настаивает на разбивке

Тогда источник данных для Рис. 2.11 нужно **переключить** с полного архива на `mchs_events_lena_bank.csv`. Разбивка будет выглядеть как 4 тонких линии на правой оси (максимум 3 события одного типа в год) — читаемо, но потеряет смысл «корреляции с общим уровнем ЧС в регионе». В этом случае правую ось нужно переименовать в «число ЧС (береговой контур Лены)» и добавить примечание об источнике.

Изменения в скрипте: функция `fig_hydro_share_vs_chs()` потребует рефакторинга — добавить цикл по типам аналогично `fig_emergency_trend()`. Это ~20 строк кода.

---

<a id="q230526-4"></a>

### Предложение 4. Таблица 3.2 — несуществующие типы ЧС

**Вопрос:** В Таблице 3.2 упоминались типы «Криогенный наледный паводок / Зажоры» и «Абразия берегов / Термопросадки грунта» — на Лене таких событий не было. Нужно убрать.

**Ответ: Уже сделано** — в предыдущей рабочей сессии (22.05.2026).

#### Текущее состояние Таблицы 3.2

Таблица полностью переписана под первичный архив `data/mchs_events_lena_bank.csv`. Из 4 оставшихся категорий ни одна не является ни зажором, ни абразией, ни термопросадкой:

| Тип ЧС | Событий | Частота/год | Пик | Макс. за сезон |
|--------|---------|-------------|-----|----------------|
| Затор (весенний ледовый) | 7 | 0,44 | Май | 3 (2018) |
| Половодье (весеннее) | 5 | 0,31 | Май | 1 |
| Чрезвычайная пожароопасность | 4 | 0,25 | Июнь–Август | 1 |
| Паводок | 1 | 0,06 | Май | 1 (2020) |
| **ИТОГО** | **17** | **1,06** | **Май** | **3 (2018)** |

#### Что было удалено и почему

| Удалённая категория | Причина |
|--------------------|---------|
| Зажоры | Отсутствуют в архиве МЧС `lena_bank` за 2008–2023 гг. Осенние инциденты в базе не зафиксированы или отнесены к иным категориям |
| Абразия берегов | Отсутствует в архиве. Это геоморфологический процесс, а не событие МЧС |
| Криогенный наледный паводок | Специальный термин, не используемый в архиве МЧС; явление относится к категории «Паводок» при наличии |
| Термопросадки грунта | Отсутствуют в архиве. Это геотехнический процесс, не фиксируемый как ЧС в рассматриваемом периоде |
| Ветер | 1 событие (октябрь 2022) — исключено как нерелевантное гидрологическим рискам и деградации ММГ |

#### Сопутствующее исправление в ВКР_ФИНАЛ.md

В §2.5.3 (`ВКР_ФИНАЛ.md`, строка 348) до сих пор написано «18 событий» — исправлено на «17» в рамках данной правки.


---

## Преемственность методологии: LC-кривые и статья-источник

> *Зафиксировано 23.05.2026 по результатам сравнительного анализа файлов
> `ВКР_ФИНАЛ.md` и `ORDERS/статья_LC_финал_11_08.pdf`.*

### Источник метода

Ключевой аналитический инструмент ВКР — **метод LC-кривых** — заимствован из следующей статьи:

> Алескеров Ф.Т., Лола И.С., Асосков Д.Г., Забелина Д.А., Назарова Р.Д.
> **«Анализ деловой неопределённости с помощью LC-кривых»**
> НИУ ВШЭ, 2024. (`ORDERS/статья_LC_финал_11_08.pdf`)

**Предметная область статьи:** экономика — Композитный индекс деловой неопределённости (ИДН) по отраслям промышленности России (2020–2024 гг.). Метод разработан для анализа **нестационарных временных рядов**, что невозможно в рамках большинства классических эконометрических подходов.

### Полное совпадение математического аппарата

| Элемент | Статья (экономика) | ВКР (геокриология / ЧС) |
|---|---|---|
| Нормирование $v_t$ к $[0;1]$ | ИДН по отрасли | гидрометеорологические ряды |
| LC-кривая (формула 3 статьи) | анализ стадий деловой активности | временна́я структура предикторов |
| Диагональ $D(t)$ (формула 4 статьи) | эталон равномерности | эталон равномерного распределения |
| $\Delta(t)$, нормировка, **4-я степень** | усилитель контраста | выделение фазового перелома |
| Финальная $LC_\Delta(t)$ | итоговая кривая | операционное окно упреждения МЧС |

ВКР воспроизводит базовую вычислительную схему **без изменений** (§2.3 ВКР_ФИНАЛ.md, явная ссылка на PDF-источник).

### Оригинальный вклад ВКР (развитие метода)

1. **Трансдисциплинарный перенос** — из экономики в геокриологию и управление рисками ЧС.
2. **Новая интерпретация** LC-кривых: опережение предиктора относительно отклика = **резерв упреждения** для перевода МЧС в режим превентивных действий (4–5 суток).
3. **Авторский индекс PSRS** (Permafrost Settlement Risk Score, 0–10 баллов) — отсутствует в статье, разработан впервые.
4. **Формула Стефана** для прогноза глубины сезонного протаивания — новый расчётный контур.
5. **Сценарная матрица реагирования** РСЧС / МЧС — прикладной управленческий инструмент, не предусмотренный экономической постановкой.

### Вывод для научного руководителя

ВКР является прямым **интеллектуальным наследником** работы Алескерова Ф.Т. и соавторов:
математический базис **сохранён и явно атрибутирован**;
содержательный вклад диссертации — в распространении аппарата на новую предметную область
и его обогащении оригинальным инструментарием геокриологии и планирования ЧС.
