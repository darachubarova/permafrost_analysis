...
cells.append(nbf.v4.new_markdown_cell(dedent(r"""# Анализ временных рядов методом LC-кривых
...
cells.append(nbf.v4.new_code_cell(dedent(r"""import pandas as pd
import numpy as np
...
cells.append(nbf.v4.new_markdown_cell(dedent(r"""## Загрузка и подготовка данных
...
cells.append(nbf.v4.new_code_cell(dedent(r"""# Путь к файлу с данными
...
cells.append(nbf.v4.new_markdown_cell(dedent(r"""## Функции для расчёта LC-кривых
...
cells.append(nbf.v4.new_code_cell(dedent(r"""def min_max_scale(series: pd.Series) -> pd.Series:
...
cells.append(nbf.v4.new_markdown_cell(dedent(r"""## Визуализация LC-кривых
...
cells.append(nbf.v4.new_code_cell(dedent(r"""# Пример построения LC-кривой для одной спецификации
...
cells.append(nbf.v4.new_code_cell(dedent(r"""# Сравнение LC-кривых для разных спецификаций (если есть колонка 'spec')
...
cells.append(nbf.v4.new_markdown_cell(dedent(r"""## Возможные расширения
