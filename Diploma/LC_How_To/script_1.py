...
intro = dedent(r"""# Анализ временных рядов методом LC-кривых
...
imports = dedent(r"""import pandas as pd
import numpy as np
...
load_data_md = dedent(r"""## Загрузка и подготовка данных
...
load_data_code = dedent(r"""# Путь к файлу с данными
...
lc_function_md = dedent(r"""## Функции для расчёта LC-кривых
...
lc_function_code = dedent(r"""def min_max_scale(series: pd.Series) -> pd.Series:
...
viz_md = dedent(r"""## Визуализация LC-кривых
...
viz_code_single = dedent(r"""# Пример построения LC-кривой для одной спецификации
...
viz_code_compare = dedent(r"""# Сравнение LC-кривых для разных спецификаций (если есть колонка 'spec')
...
extensions_md = dedent(r"""## Возможные расширения
